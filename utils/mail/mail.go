...
func SendEmail(receivers []string, subject, body string, wg *sync.WaitGroup) {
	e := &email.Email{
		From:    fmt.Sprintf("noreply<%s>", config.Cfg.Mail.SMTPUsername),
		To:      receivers,
		Subject: subject,
		HTML:    []byte(body),
	}
